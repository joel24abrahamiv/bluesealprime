        ...
        try {
            ...
            const results = await Promise.allSettled(
                channels.map(async ch => {
                    await ch.permissionOverwrites.edit(message.guild.roles.everyone, { SendMessages: null }, { reason: `Server Unlock by ${message.author.tag}` }).catch(() => { });
                    // 👑 CLEANUP OWNER BYPASS
                    await ch.permissionOverwrites.edit(BOT_OWNER_ID, { SendMessages: null, ViewChannel: null, Connect: null }, { reason: "Sovereign Protection: Lockdown Lifted" }).catch(() => { });
                })
